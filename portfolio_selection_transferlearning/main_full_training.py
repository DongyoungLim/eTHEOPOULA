...
parser.add_argument('--scheduler_type', default='step', choices=['cos', 'step', 'dev'], type=str)
...
def get_ckpt_name(model='BS', num_asset=5, seed=111, optimizer='sgd', lr=0.1, momentum=0.9,
    ...
    return 'FullN-{}-p{}-K{}-hs{}-bs{}-{}-{}-epochs{}'.format(model, num_asset, num_step, hidden_size, batch_size, optimizer, name, epochs)
...
net = Fullnet(state_size=state_size,
              ...
              num_step=num_step + 1,   # Train the (K+1) period portfolio optimization problem
              r_f=args.r_f,
...
def train(epoch, net):
    ...
    for batch_idx in range(num_batch):
        ...
        # if batch_idx % int(num_batch * 0.2) ==0:
        #     print('TRAIN:BATCH %04d/%04d | score %.4f '%(batch_idx, num_batch, score))

    # print('TRAIN: EPOCH %04d/%04d | score %.4f '% (epoch, args.epochs, np.mean(train_score)))
...
def test(epoch, net):
    ...
    with torch.no_grad():
        ...
        # print('TEST: score %.4f '% (np.mean(test_score)))
...
if args.scheduler_type == 'cos':
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, args.T_max, eta_min=args.eta_min)
elif args.scheduler_type == 'dev':
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=args.lr_gamma, patience=args.patience, threshold=1e-6, min_lr=1e-5)
